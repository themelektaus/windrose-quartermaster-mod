...
namespace Windrose.Quartermaster.Core.Deploy
{
    public sealed class GameDeployer
    {
        ...
        // DLL sidecar for binary-DataAsset multipliers (tree harvest, dig volumes).
        // Loot-table min/max is handled entirely by the pak JSON overrides.
            ...
            double digVolumeMult = 1.0)
        {
            ...
            if (treeMult == 1.0 && digVolumeMult == 1.0)
            {
                if (File.Exists(path))
                {
                    LogLine("Removing qm_loot_" + profileSafeName + ".json (no multipliers) -> " + path);
                    File.Delete(path);
                }
                return;
            }
            LogLine("Writing qm_loot_" + profileSafeName + ".json (tree x" +
                treeMult.ToString("F2") + ", digvol x" + digVolumeMult.ToString("F2") +
                ") -> " + path);
            EnsureSidecarDir();
